...
Azure App Service (containerized) + Azure SQL Database with GitHub OAuth authentication.
...
All database traffic flows over a **Private Endpoint** — Azure SQL is never exposed to the
public internet, with Entra ID-only authentication.
...
- Private Endpoint only — `publicNetworkAccess: 'Disabled'` on Azure SQL
- Azure SQL authentication — Entra ID only (`azureADOnlyAuthentication: true`)
