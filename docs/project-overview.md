...
Orchestrated by the **16-App Conductor**, separate from infrastructure:

```text
A1: Scaffolder        → Turborepo + Next.js monorepo
A2: API Builder       → Auth middleware, role guards
A3-A5: API Builder    → Hackathons, scoring, challenges, admin routes
A6: API Builder       → Full API review gate
A7: Frontend Builder  → Pages and components
A8: Test Writer       → Unit + integration tests
A9: App Deployer      → CI/CD workflows
```
