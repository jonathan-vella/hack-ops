...
export async function buildLeaderboard(
  ...
  const hackathon = await queryOne<HackathonDoc>(
    "SELECT id, name, status FROM hackathons WHERE id = @hid",
    { hid: hackathonId },
  );
  ...
  // Parallel queries
  const [scores, teams, challenges] = await Promise.all([
    query<ScoreDoc>(
      "SELECT id, teamId, challengeId, total, approvedAt FROM scores WHERE hackathonId = @hid",
      { hid: hackathonId },
    ),
    query<TeamDoc>(
      "SELECT id, name, hackathonId FROM teams WHERE hackathonId = @hid",
      { hid: hackathonId },
    ),
    query<ChallengeDoc>(
      "SELECT id, title, maxScore, [order] FROM challenges WHERE hackathonId = @hid ORDER BY [order] ASC",
      { hid: hackathonId },
    ),
  ]);
