...
function AdminDashboard() {
  ...
  // Admin needs a hackathonId for the submissions API (required by role guard)
  const firstActiveId = hackathons.find((h) => h.status === "active")?.id;
  const pendingUrl = firstActiveId
    ? `/api/submissions?hackathonId=${firstActiveId}&status=pending`
    : null;
  ...
  const totalTeams = hackathons.reduce((acc, h) => acc + h.teamCount, 0);
...
function CoachDashboard() {
  ...
  const pendingUrl = selectedHackathon
    ...
    : null;
  const { data: submissions, refetch } = useFetch<{
    items: SubmissionsAPI.SubmissionRecord[];
  }>(pendingUrl);
...
function HackerDashboard() {
  const { data: progression } =
    useFetch<ProgressionAPI.ProgressionRecord>("/api/progression");
  const hackathonId = progression?.hackathonId;

  const challengeUrl = hackathonId
    ? `/api/challenges?hackathonId=${hackathonId}`
    : null;
  const { data: challengeData } = useFetch<{
    items: ChallengesAPI.ChallengeRecord[];
  }>(challengeUrl);

  const teamUrl = hackathonId ? `/api/teams?hackathonId=${hackathonId}` : null;
  const { data: teamData } = useFetch<{ items: TeamsAPI.TeamRecord[] }>(
    teamUrl,
  );
  ...
  return (
    <div className="space-y-6">
      ...
      {currentChallenge && <SubmissionForm challengeId={currentChallenge.id} />}
