  ...
  async (request: NextRequest, context, auth) => {
    ...
    if (body.status === "approved") {
      if (!body.scores || body.scores.length === 0) {
        return NextResponse.json(
          {
            error: "Scores are required when approving a submission",
            ok: false,
          },
          { status: 400 },
        );
      ...
      // LOGIC-005: Require complete rubric category coverage
      const categoryIds = new Set(categories.map((c) => c.id));
      const scoredIds = new Set(
        body.scores.map((s: CategoryScore) => s.categoryId),
      );
      if (
        categoryIds.size !== scoredIds.size ||
        ![...categoryIds].every((id) => scoredIds.has(id))
      ) {
        return NextResponse.json(
          {
            error: `All rubric categories must be scored. Expected: ${[...categoryIds].join(", ")}`,
            ok: false,
          },
          { status: 400 },
        );
      ...
      // LOGIC-006: Wrap approval path in transaction for atomicity
