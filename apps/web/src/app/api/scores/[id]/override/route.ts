...
export const PATCH = requireAuth(
  async (request: NextRequest, context, auth) => {
    ...
    // Preserve original scores in audit trail
    ...
    // LOGIC-007: Insert new override record and mark original as superseded
    const overrideId = `score-override-${crypto.randomUUID().slice(0, 8)}`;

    await transaction(async (tx) => {
      // Mark original as overridden
      await tx.execute(
        `UPDATE scores
       SET overriddenBy = @overriddenBy, overriddenAt = @overriddenAt, overrideReason = @overrideReason
       WHERE id = @id`,
        {
      ...
      // Insert new score record with override linkage
      await tx.execute(
