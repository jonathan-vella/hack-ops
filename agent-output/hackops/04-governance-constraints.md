...
| MCAPSGov Deny — SQL AD-only     | Management Group | Deny   | Azure SQL/SQL MI without AD-only auth                                    | Checks `azureADOnlyAuthentication` property                                          | N/A (SQL not used)                                             | N/A             |
...
- All MCAPSGov Deny policies target VM/AKS/OpenAI/SQL resources that
  are not part of the HackOps architecture
- The CosmosDB Modify policy is the most impactful — it forces
  RBAC-only authentication, eliminating connection string access
...
| Cosmos DB auth   | Local auth disabled (keys/connection strings)| Managed identity + SQL RBAC role assignment                 | ✅     |
...
| Cosmos DB connection string auth | CosmosDB_LocalAuth_Modify      | Modify | Switched to Entra ID RBAC: managed identity + SQL role assignment for App Service |
...
| CosmosDB_LocalAuth_Modify     | Modify            | `disableLocalAuth: true` set on Cosmos DB account     |
...
| CosmosDB_LocalAuth_Modify     | Modify | Cosmos DB `disableLocalAuth` set to `true`                |
...
by HackOps (classic ASM, VM SKUs, AKS, OpenAI, SQL/SQL MI). The
resource group tag Deny policy is addressed by including all 9
required tags.
...
| Public Access                | Cosmos DB: `publicNetworkAccess: Disabled` (architecture decision)    |
| Managed Identity             | Cosmos DB RBAC via system-assigned managed identity (policy-enforced) |
...
| Cosmos DB Local Auth         | Disabled — policy auto-sets `disableLocalAuth: true`                  |
